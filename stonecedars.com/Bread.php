...
<body>
  ...
  $result = mysqli_query($conn, "SELECT * FROM bread");
  while ($row = mysqli_fetch_assoc($result)) {
    echo '<div class="card">';
    echo '<img src="pics/' . strtolower($row["bread_name"]) . '.jpg" alt="' . $row["bread_name"] . '">';
    echo '<h2>' . $row["bread_name"] . '</h2>';
    echo '<p>$' . $row["price"] . '</p>';
    echo '<button onclick="window.location.href=\'add_to_cart_bread.php?id=' . $row['id'] . '\'">Add to Cart</button>';
    echo '</div>';
  }
