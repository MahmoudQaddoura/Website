...
<body>
  ...
  $result = mysqli_query($conn, "SELECT * FROM pastries");
  while ($row = mysqli_fetch_assoc($result)) {
    echo '<div class="card">';
    echo '<img src="pics/' . strtolower($row["pastry_name"]) . '.jpg" alt="' . $row["pastry_name"] . '">';
    echo '<h2>' . $row["pastry_name"] . '</h2>';
    echo '<p>$' . $row["price"] . '</p>';
    echo '<button onclick="window.location.href=\'add_to_cart_pastries.php?id=' . $row['id'] . '\'">Add to Cart</button>';
    echo '</div>';
  }
